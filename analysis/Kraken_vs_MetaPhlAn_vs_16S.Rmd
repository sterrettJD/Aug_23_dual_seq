...
k.feature.table.relative <- k[,rel.cols]
colnames(k.feature.table.relative) <- gsub(
    colnames(k.feature.table.relative), 
    pattern=".bracken_frac", replacement="")

num.cols <- colnames(k)[grepl(x=colnames(k), pattern="_num")]
k.feature.table.num <- k[,num.cols]
colnames(k.feature.table.num) <- gsub(
    colnames(k.feature.table.num), 
    pattern=".bracken_num", replacement="")


k.pseq <- phyloseq(otu_table(k.feature.table.relative, taxa_are_rows=T),
                   tax_table(as.matrix(k.tax.table)),
