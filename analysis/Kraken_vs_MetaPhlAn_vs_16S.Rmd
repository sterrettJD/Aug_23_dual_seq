...
metadata.16s <- data.table::fread("../16s-data/16S_meta.txt", data.table=F)
...
participant.metadata$HIV <- participant.metadata$`What is the participant's HIV status?`
...
participant.metadata[participant.metadata$Sample=="DM001", "MSM"] <- TRUE
participant.metadata[participant.metadata$PID=="DM002", "MSM"] <- TRUE
participant.metadata[participant.metadata$PID=="DM007", "MSM"] <- TRUE
participant.metadata[participant.metadata$PID=="DM093", "MSM"] <- FALSE

participant.metadata$MSM.Label <- ifelse(participant.metadata$MSM,
                             yes="MSM",
                             no="MSW")

metadata <- merge(metadata, metadata.16s[c("Sample_ID", "HIV_Status", "Cohort")],
...
## By Cohort {.tabset}
### 16S
```{r 16s_phy_genus_microshades_coh, fig.width=8, fig.height=10}
make_microshades(s.pseq.nohost, facet_by="Cohort")
```

### Kraken
```{r kraken_phy_genus_microshades_coh, fig.width=8, fig.height=10}
make_microshades(k.pseq.nohost, biggroup="phylum", subgroup="genus", 
                 facet_by="Cohort")
```

### MetaPhlAn
```{r metaphlan_phy_genus_microshades_coh, fig.width=8, fig.height=10}
make_microshades(m.pseq, biggroup="Phylum", subgroup="Genus", 
                 facet_by="Cohort")
```
